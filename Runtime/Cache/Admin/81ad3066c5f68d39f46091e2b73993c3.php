	...
	<div class="con" id="content">
			<table class="table margt20">
				...
				<?php if(is_array($table_data)): $k = 0; $__LIST__ = $table_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($k % 2 );++$k;?><tr>
						<td><?php echo ($k); ?>.</td>
						<?php if(is_array($data_index)): $i = 0; $__LIST__ = $data_index;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$index): $mod = ($i % 2 );++$i; if($index == 'astate'): if($data["astate"] == 'auditing'): ?><td><a class="abtn abtn-fc">外审中</a></td>
								<?php elseif($data["astate"] == 'access'): ?>
									<td><a class="abtn abtn-fc" onclick="showDialog('auditor_access','<?php echo ($data["auditor"]); ?>','<?php echo ($data["muid"]); ?>');">外审通过</a></td>
								<?php elseif($data["astate"] == 'fail'): ?>
									<td><a class="abtn abtn-fc" onclick="showDialog('auditor_fail','<?php echo ($data["auditor"]); ?>','<?php echo ($data["muid"]); ?>');">外审不通过</a></td>
								<?php else: ?>
