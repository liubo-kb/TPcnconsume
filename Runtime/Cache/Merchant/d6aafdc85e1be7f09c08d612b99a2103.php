...
<link rel="stylesheet" href="/cnconsum/Public/css/merchant/good.css" />
<script src="/cnconsum/Public/js/merchant/pages.js"></script>
<script src="/cnconsum/Public/js/merchant/zDialog.js" type="text/javascript"></script>
<script src="/cnconsum/Public/js/merchant/dialog.js"></script>
    	...
    	<div class="navinfo"><img src="/cnconsum/Public/image/merchant/ihy.png" id="ihy"/>&nbsp;&nbsp;我的商品</div>
    		<div class="hang" >
                <form action="" method="post">
                   	<a href="addView" class="abtn">添加商品</a>
	    				<label class="searchbar" style="display:none">
				    		<input type="text" class="srctxt" name="search" value="" placeholder="搜索" />
				    		<input type="submit" value=" " class="srcbtn" />
	    				</label>
    			</form>
    		</div>
    		...
    		<div class="cmain">
	            <div class="cinfo" id="content">
					...
					<?php if(is_array($table_data)): $i = 0; $__LIST__ = $table_data;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$data): $mod = ($i % 2 );++$i;?><tr>
							...
							<td><a class='abt' href="modView?code=<?php echo ($data["number"]); ?>">编辑</a></td>
							<td><a class='abt' href="javascript:delCommodity('确定要删除这条信息吗？',<?php echo ($data["number"]); ?>)">删除</a></td>
