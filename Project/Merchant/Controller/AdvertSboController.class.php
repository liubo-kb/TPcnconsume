...
use Merchant\Model\AdvertSboModel;
...
class AdvertSboController extends Controller 
{
	...
	public function add()
	{
		...
		$record = array(
			'merchant' => post('muid'),
			'advert_content' => post('advert_content'),
			'online_date' => post('online_date'),
			'stay_time' => post('state_time'),
	...
	public function onlineAdvertGet($advert,$where)
	{
		$online_advert = $advert
                ->field('store,content,address,image_url,merchant,longtitude,latitude')
	...
	public function onlineMerchantGet($subWhere = "",$where = "")
	{
		...
		$merchant = M();
                $subQuery = "(select merchant from cn_advert_sbo where state = 'true' and ".$subWhere.") LIMIT 0,10";
               	$query = "select store,address as content,address,image_url,phone,longtitude,latitude from cn_merchant where ".$where." state = 'true' and phone not in ";
