...
class MainController extends Controller 
{
	...
	public function insure()
    {
		...
		$set['state'] = 'true';
		setWithCheck($table,$where_m,$set);

		$table = D('advert_activity_list');
		$where['id'] = '1';
		$table->where($where)->setInc("position");
		$record = array(
				'id' => '1', 'muid' => $muid, 'position' => '1','title' => $info['store'], 'info' => '最新店铺入驻',
				'image_url' => $info['image_url'], 'pay_type' => 'click','pay_content' => '100000',
				'remain' => '100000','sum' => '100000','datetime' => currentTime(), 'state' => 'true'
		);
		...
		$record = array(
			'account' => '周思梦', muid => $muid, 'tip' => '保险认证通过',
	...
	public function inner()
	{
		...
		$table = D('audit_result');
		$record = array(
			'account' => '周思梦', muid => $muid, 'tip' => '快速认证通过',
			'state' => 'complete_not_auth', 'datetime' => currentTime()
		);
		$result = addWithCheck($table,$record);
	...
	public function auditFail()
	{
		...
		$table = D('audit_result');
		$record = array(
				'account' => 'liubo', muid => $muid, 'tip' => $tip,
	...
	public function settle()
	{
		...
		$data = D('merchant');
		$where['state'] = array('in',array('null','auditing'));               
	...
	public function audited()
    {
		...
		$where['cn_merchant.state'] = array('in',array('true','false','complete_not_auth'));		
		...
		$table_data = $data
		->where($where)
		->order('cn_audit_result.datetime desc')
		->join("cn_audit_result on cn_merchant.muid = cn_audit_result.muid")
	...
	{
		$record = array(
			'account' => '周思梦','tip' => post('reason'),'muid' => post('muid'),
			'state' => 'false','datetime' => currentTime()
		);
		$table = D('audit_result');
		$result = addWithCheck($table,$record);
