...
class AdvertTopController extends Controller 
{
	...
	public function get()
	{
		...
		{
			$result[$i]['num'] = $this->getNum($result[$i]['id']);
		}
	...
	public function add()
	{
		...
		$sum = advertPay($pay_type,'top',$para);
	...
	public function set()
	{
		...
		logIn(json_encode($where));

		$set['datetime'] = currentTime();
		$set['state'] = "ONLINE";
		D('advert_top_list')->where($where)->save($set);
	}
	...
	public function del()	
	{
		$where['id'] = post('id');
                $where['muid'] = post('muid');
                $where['position'] = post('position');
		$result['result_code'] = D('advert_top_list')->where($where)->delete();
		echo json_encode($result);
