			...
			if(isWeiXin()){
				...
				if($scope.state == "login_access"){
					   ...
					   console.log("不是来自微信内置浏览器")
						...
						$state.go('goods',{list:JSON.stringify(cardList),name:name,type:JSON.stringify(newObj),num:'0'});
				}else{
...
module.controller('goodsCtrl',function($scope,$stateParams,$http,$state,$ionicPopup,$timeout){
	...
	vm.goRedPacket = function(mak,num){
		if(mak == "markRed"){
			vm.mark = !"markRed"
			vm.cardType = ""
		}else{
			$state.go("redPacket",{list:JSON.stringify(vm.cardList),name:vm.storeName,type:JSON.stringify(vm.cardType),num:num},{location:"replace"})
		}
	}
	
	//使用商家优惠券
	
	vm.goMerQuan = function(mak,num){
		if(mak == "markMer"){
			vm.mark = !"markMer"
			vm.cardType = ""
		}else{
			$state.go("merQuan",{list:JSON.stringify(vm.cardList),name:vm.storeName,type:JSON.stringify(vm.cardType),num:num},{location:"replace"})
		}
	}
	
	//使用商消乐优惠券 
	
	vm.goPlatQuan = function(mak,num){
		if(mak == "markPlat"){
			vm.mark = !"markPlat"
			vm.cardType = ""
		}else{
			$state.go("platQuan",{list:JSON.stringify(vm.cardList),name:vm.storeName,type:JSON.stringify(vm.cardType),num:num},{location:"replace"})
		}
	...
	if(vm.cardType == "unit" || vm.cardType == "set" || vm.cardType == ""){
		...
		vm.mark = "markMer"     //商家优惠券
	  	vm.delog = 'cp#办卡#'+vm.unit.code+'#'+vm.unit.level+'#'+vm.unit.type+'#card_temp_color#'+vm.uuid+'#'+vm.unit.merchant+'#'+(vm.unit.price-vm.cardType.sum)+'#'+vm.cardType.coupon_id
	}else if(vm.cardType.id){
	    vm.mark = "markPlat"    //商消乐优惠券
	  	vm.delog = 'scp#办卡#'+vm.unit.code+'#'+vm.unit.level+'#'+vm.unit.type+'#card_temp_color#'+vm.uuid+'#'+vm.unit.merchant+'#'+vm.unit.price+'#'+vm.cardType.coupon_id
	...
	//结算页面，点击跳转；
	vm.alipay = function(mak,item,type){
		...
		if(mak==""){
			...
			vm.delog = 'null#办卡#'+item.code+'#'+item.level+'#'+item.type+'#card_temp_color#'+vm.uuid+'#'+item.muid+'#'+item.price
		}else if(mak == "markRed"){
			var itemPrice = Number(item.price)*0.9;
			if(itemPrice > type){
				vm.newPrice = type;
				vm.fina = Number(item.price)-vm.newPrice;
				vm.delog = 'rp#办卡#'+item.code+'#'+item.level+'#'+item.type+'#card_temp_color#'+vm.uuid+'#'+item.muid+'#'+item.price+'#'+vm.newPrice
			}else{
			...
			vm.fina = Number(item.price)-Number(type.sum);
			vm.delog = 'cp#办卡#'+item.code+'#'+item.level+'#'+item.type+'#card_temp_color#'+vm.uuid+'#'+item.muid+'#'+(item.price-type.sum)+'#'+type.coupon_id
		}else if(mak == "markPlat"){
			vm.fina = Number(item.price)-Number(type.sum);
			vm.delog = 'scp#办卡#'+item.code+'#'+item.level+'#'+item.type+'#card_temp_color#'+vm.uuid+'#'+item.muid+'#'+item.price+'#'+type.coupon_id
		}
		
		console.log(vm.delog);
		console.log(vm.fina);
			...
			$http.post(url, data, postCfg).success(function (response) {
					...
					if(vm.storeSum < item.price){
				    	...
				    	$http.post(url, data, postCfg).success(function (response) {
				    			...
				    			if(vm.cardData.result_code=="false"){
				    				$state.go("payCard",{id:vm.delog,type:vm.fina})
				    			}else{
				    				hint("您已拥有此卡")
				    			}
...
module.controller("redPacketCtrl",function($scope,$stateParams,$http,$state){
	...
	//未选择红包，跳转结算页面
	vm.goGoods = function(){
		$state.go('goods',{list:JSON.stringify(vm.cardList),name:vm.storeName,type:JSON.stringify(vm.cardType),num:vm.index},{location:'replace'})
	}
	...
	//获得价格后,跳转结算页面
	 vm.shopRed = function(sum){
	 	vm.markShow = true;
		$state.go('goods',{list:JSON.stringify(vm.cardList),name:vm.storeName,type:JSON.stringify(sum),num:vm.index},{location:'replace'})
...
module.controller("merQuanCtrl",function($scope,$stateParams,$http,$state){
	...
	//获得价格后,跳转结算页面
	 vm.getMerQuan = function(obj){
		$state.go('goods',{list:JSON.stringify(vm.cardList),name:vm.storeName,type:JSON.stringify(obj),num:vm.index},{location:'replace'})
	 }
...
module.controller('allCouCtrl',function($scope,$stateParams,$http,$state){
    ...
    vm.getCou = function(num){
		console.log(num)
		var newArr = [];
		newArr.push(vm.cardInfo)
		$state.go('goods',{list:JSON.stringify(newArr),name:vm.storeName,type:JSON.stringify(num)},{location:'replace'})
	}
