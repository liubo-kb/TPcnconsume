...
fter.filter('dataCardInfo',function(){
	return function(obj){
    	if(obj.card_type == '套餐卡'){
    		var objArr = JSON.parse(obj.content)
    		var shopCont = objArr[2];
    		var shopName = shopCont.item;
    		var shopItem = shopCont.value;
    		var newStr = shopName+':'+shopItem;
    		return newStr
    	}else{
    		return
    	}
...
fter.filter('dataResult',function(){
	return function(obj){
		if(obj.card_type != '计次卡'){
			var newStr = obj.sum + "元";
			return newStr
		}else{
			var objArr = JSON.parse(obj.content)
    		var shopCont = objArr[3];
    		var newStr = shopCont.value + "次";
			return newStr
		}
	}
})
...
fter.filter("userState",function(){
	return function(str){
		if(str == 'not_auth'){
			return "未认证"
		}else if(str == 'access'){
